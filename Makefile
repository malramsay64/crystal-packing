...
lint: venv ## check style with flake8
	venv/bin/python -m flake8 pypacking tests
...
coverage: venv ## check code coverage quickly with the default Python
	venv/bin/python -m coverage run --source pypacking -m pytest
...
docs: venv ## generate Sphinx HTML documentation, including API docs
	rm -f docs/pypacking.rst
	rm -f docs/modules.rst
	venv/bin/python -m sphinx-apidoc -o docs/ pypacking
	$(MAKE) -C docs clean
	$(MAKE) -C docs html
